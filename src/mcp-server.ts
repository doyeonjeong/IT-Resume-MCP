...
// `quiet: true` suppresses dotenv's stdout banner (e.g. "[dotenv@x] injecting env from .env").
// MCP uses stdout as the JSON-RPC channel — any banner text breaks the protocol.
...
async function bootstrap() {
  ...
  // hit the wire). Silence Nest's logger so only JSON-RPC messages go to
  // stdout. Diagnostics still go to stderr via console.error if needed.
